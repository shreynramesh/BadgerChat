...
a. Simple and Intuitive Use: There are no unecessary steps when logging in, logging out and creating a post. Everything happens with a click of the button.
b. Low Physical Effort: There are no unecessary movements when logging in, logging out, creating, and deleting a post. Again, everything happens with just moving the mouse and a few keystrokes
Principles we may have violated:
a. Equitable Use: Our website wil not be very accessable to people who cannot see since most interactions are with the mouse. We would need a screen reader to do this. Our website is also not accessable to people who may have a comprimised ability to type due to cognitive or physical impairments.
b. Tolerance for Error: If a user accidently deletes one of their posts, they have no way of getting it back without make a new post entirely. No directions or prompts are provided for that. One solution to this is an undo prompt.
...
![Accessability Sketch](_figures/sketch.png)
